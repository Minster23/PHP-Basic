  ...
  <div class="w-full h-auto">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
      ...
      if (isset($_GET['keyword']) && !empty($_GET['keyword'])) {
        ...
        $query = "SELECT * FROM tb_produk WHERE Nama LIKE '%$keyword%'";

        // Execute the query
        $result = mysqli_query($koneksi, $query);

        // Check if any products match the search keyword
        if (mysqli_num_rows($result) > 0) {
          while ($r2 = mysqli_fetch_array($result)) {
            $id         = $r2['id'];
            $ffoto         = base64_encode($r2['foto']);
            $nama        = $r2['Nama'];
            $deskrisi        = $r2['Deskripsi'];
            $harga        = $r2['Harga'];
      ?>
            <div class="">
              <div class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                <?php echo "<img src='data:image/jpeg;base64,$ffoto' class=class='w-full h-auto max-w-xl rounded-lg'>"; ?>

                <a href="http://localhost/apaaja/client/detail.php?id=<?php echo $id; ?>" class="block p-6 bg-white rounded-lg">
          ...
          <div id="alert-border-2" class="flex p-4 mb-4 text-red-800 border-t-4 border-red-300 bg-red-50 dark:text-red-400 dark:bg-gray-800 dark:border-red-800" role="alert">
            ...
            <div class="ml-3 text-sm font-medium">
              Kesalahan pencarian <a href="jelajahi.php" class="font-semibold underline hover:no-underline">Kembali</a>. Kembali ke beranda.
            </div>
